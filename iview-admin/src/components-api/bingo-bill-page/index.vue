    ...
    <div class="bill-component">
        ...
        <div class="bill" :style="'height:'+ 'calc(100% - '+sHeight+'px)' "  :class='billClass'>
            ...
            <div :class='[mode=="horizontal" ? "ivu-hor" : "ivu-fr"]'>
                <Menu ref="billMenu" :active-name="activeName" width="130px" :mode="mode" @on-select="handleScroll" @on-open-change="selectClick()">
            ...
            <div :class="contentClass" ref="content" v-scroll="loadMore">
                <Card class="content-card" :class='[selectName==tab.name && fullFlag ? "card-full" : "",tab.expend||(mode=="horizontal")?"":"expendClass"]'
                      :ref="tab.name" v-if="tab.show" v-for="(tab, key) in tabs" :key="key">
                    <p slot="title" v-if="mode=='vertical'"><Icon :type="tab.icon" class="ivu-mr-8"/>{{tab.title}}
                        <template>
                            <Button icon="ios-arrow-down" class="shrink-card-icon" v-if="tab.expend" @click="showTab(tab)"></Button><!-- 展开 -->
                            <Button icon="ios-arrow-up" class="shrink-card-icon" v-if="!tab.expend" @click="showTab(tab)"></Button><!-- 展开 -->

                            <Button icon="md-resize" class="shrink-card-icon1" v-if="!fullFlag" @click="fullOpen(tab.name)"></Button>
                            <Button icon="icon iconfont iconweibiaoti11" class="shrink-card-icon1" v-if="fullFlag" @click="fullOpen(tab.name)"></Button>
                        </template>
                    </p>
