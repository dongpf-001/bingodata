<template>
    <div class="i-table-border list-page-out list-resize bingo-advance"
         :style="'height:'+ 'calc(100% - '+sHeight+'px)' " ref="table-list" v-resize="handleResize"
         :class="{'fullScreen': fullFlag}">
        <div v-show="toolbar" ref="toolbar">
            <bingo-toolbar :showNew="showNew" :showDelete="showDelete" :showRefresh="showRefresh"
                           ...
                           @on-delete="onDelete" @open-full-modal="openFullModal" :fullFlag="fullFlag"
                           :disabledNew="disabledNew" :disabledDelete="disabledDelete"
            ...
            <span slot="toolbar">
             <slot>
                ...
                <Select v-if="layoutShow" v-model="layoutValue" style="width:150px;margin-left: 5px" placeholder="请选择布局"
        ...
        <div :class="{'table-full': fullFlag}">
            <Spin size="large" fix v-if="loading" :style="'margin-top: 50px;height:calc(100% - '+loadingToB+'px)' ">
            
            </Spin>
            <ag-grid-vue class="table ag-theme-fresh"
                         rowHeight="36"
                         headerHeight="36"
                         v-bind="$attrs"
                         v-on="$listeners"
        ...
        <bingoModalEdit v-model="layoutModal" :width="860" title="布局管理">
           <span slot="toolbar">
                   <Button type='primary' customIcon='iconfont iconxinjian1' @click="addLayout">新建</Button>
                    ...
                    <Button type='primary' customIcon='iconfont iconbaocunbingguanbi'
                            @click="saveLayoutClose">保存并关闭</Button>
...
<script>
    ...
    export default {
        ...
        components: {
            ...
            bingoModalEdit,
            'CurrencyComponent': {
                template: '<span><Input @on-change="params.context.componentParent"  placeholder="公司名称" style="width: 100px"/> {{params.value}}</span>',
                methods: {
                    valueCubed() {
        ...
        beforeMount() {
            this.gridOptions.context = {
                componentParent: this.$parent.$parent
            }

            this.getDataPath = data => {
                return data[this.treeColumn]
            }
        },
        computed: {},
        watch: {
        ...
        methods: {
            layoutChange(layoutValue) {
                ...
                } else if (this.baselayoutInfo.length != 0) {
                    this.gridApi.columnController.setColumnState(this.baselayoutInfo)
                }
            },
            syncCurrentLayout() {
                ...
                for (let column of this.currentLayout) {
                    if (column.colId == '0') {
                        column.colId = '多选框'
                        column.colName = '多选框'
                    } else if (column.colId == '1') {
                        column.colId = '序号'
                        column.colName = '序号'
                    }
                    for (let item of this.columnDefs) {
                        if (column.colId == item.field) {
                            column.colName = item.headerName
                        }
                    }
                }
            ...
            getLayoutInfo(layoutValue) {
                let params = {
                    userId: this.$store.state.admin.user.info.data.userId,
                    layoutName: layoutValue,
                    tableName: this.layoutName
                }
            ...
            getLayout() {
                ...
                Api.getUserLayout(params).then(res => {
                    ...
                    this.baselayoutInfo = this.gridApi.columnController.getColumnState()
                })
            },
            delLayout() {
                if (this.layoutValue == '') {
            ...
            sortChanged(data) {
                let columns = data.columnApi.columnController.gridColumns
                for (let column of columns) {
                    for (let col in column) {
                        if (col == 'sort' && column[col]) {
                            this.orderBy = column.colId + ' ' + column.sort
                        }
                    }
                }
            ...
            toChangeData(datas, treeColumn) {
                let newData = {}
                for (let d in datas) {
                    if (d != 'children') {
                        if (d == this.treeColumn) {
                            if (treeColumn) {
                                if (Array.isArray(datas[d])) {
                                    newData[d] = [...[treeColumn], ...datas[d]]
                                } else {
                                    newData[d] = [...[treeColumn], ...[datas[d]]]
                                }
                            } else {
                        ...
                        for (let children of datas.children) {
                            this.toChangeData(children, datas[this.treeColumn])
                        }
                    }
                }
            ...
            onGridReady_column(params) {
                ...
                this.columnApi_column = params.columnApi
