<template>
    <div class="i-table-border list-page-out list-resize" :style="'height:'+ 'calc(100% - '+sHeight+'px)' "
         ref="table-list" v-resize="handleResize" :class="{'fullScreen': fullFlag}">
        <div v-show="toolbar" ref="toolbar">
            <bingo-toolbar :showNew="showNew" :showDelete="showDelete" :fullFlag="fullFlag"
                           :showRefresh="showRefresh" :showAllButton="showAllButton" :showOpenButton="showOpenButton"
                           @on-new="newClick" @on-refresh="onRefresh" @on-delete="onDelete"
                           @open-full-modal="openFullModal"
                           :disabledNew="disabledNew" :disabledDelete="disabledDelete"
                           :disabledRefresh="disabledRefresh">
            <span slot="toolbar">
             <slot></slot>
                 <Select v-if="layoutShow" v-model="layoutValue" style="width:150px;margin-left: 5px"
                         placeholder="请选择布局" @on-change="layoutChange">
