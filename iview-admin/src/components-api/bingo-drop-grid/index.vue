<template>
    <div style="width: 100%;display:inline-block;"  v-resize="handleResize" class="drop-grid">
        ...
        <Tooltip :content="modelString" :disabled="(model==''||!model)||!multiple" max-width="800" style="width: 100%">
            <Select class="drop-select" :max-tag-count="maxTag" :max-tag-placeholder="maxTagPlaceholder" v-bind="$attrs" v-on="$listeners" :transfer="transfer" v-model="model" placeholder="" style="width:100%;white-space:nowrap;" filterable :clearable="clearable"  :disabled="disabled"   :multiple="multiple" :remote-method="remoteMethod"
                     @on-change="change" @on-open-change="openChange" ref="select">
                <Option v-for="item in selectList" :value="item[keyValue]" class="drop-grid ivu-hide" >{{item[keyName]}}</Option>
    ...
    getSelectList () {
      ...
      if (this.value && this.value != '') {
        ...
        if (this.assignmentFlag) {
          ...
          this.$emit('getData', params, function (res) {
            _this.selectList = _this.selectList.concat(res.data.rows);
            [ { 'id': 15159, 'fax': '', 'degree': '本科', 'version': 'Vnull', 'nachn': '', 'stell': '50032038', 'name': '杨可凡', 'isHistory': '', 'phoneSstp': '', 'displayName': '', 'creatorId': '', 'idCodeRight': '304130', 'ortOne': '', 'orgeh': '50129851', 'subGroupCode': '1N', 'gradDate': '2015-08-04', 'department': 'TAC1', 'officeLocal': '', 'email': 'YangKefan@CSVW.COM', 'pernr': '12335' } ]
            if (_this.multiple) {
              _this.getData()
            }
          })
